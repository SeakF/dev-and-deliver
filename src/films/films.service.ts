...
export class FilmsService {
  ...
  async findMostPopularCharacterInOpenings() {
    ...
    const characterNames =
      await this.peopleService.getAllNamesWithoutPagination();
  ...
  private async getAllOpeningCrawls(currentPage = 1): Promise<string[]> {
    ...
    if (films.isNextPage) {
      openingCrawlArrayFromAllFilms.push(
        ...(await this.getAllOpeningCrawls(++currentPage)),
      );
    }
  ...
  private countWords(parsedOpeningCrawls: string[]) {
    ...
    for (const parsedOpeningCrawl of parsedOpeningCrawls) {
      ...
      for (const word of words) {
        const clearWordFromPunctuation = word
          .replace(/[.,!?;:'"(){}\[\]\-]+$/, '')
          .trim();
  ...
  private findMatchingNames(names: string[], parsedOpeningCrawls: string[]) {
    ...
    const resultNames = Object.keys(occurrences).filter(
      (name) => occurrences[name] == mostOccurrences,
    );
